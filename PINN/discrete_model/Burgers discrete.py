...
class PhysicsInformedNN():
    ...
    def net_U0(self, x):
        ...
        u_x0 = torch.autograd.grad(
            u, x,
            grad_outputs=torch.ones_like(self.dummy),
            retain_graph=True,
            create_graph=True,
            only_inputs=False
        )[0]
        u_xx0 = torch.autograd.grad(
            u_x0, x,
        ...
        u_x = self.fwd(u, x)
        u_xx = self.fwd(u_x, x)
        sum_ux = torch.sum(u_x, dim=1)
        sum_uxx = torch.sum(u_xx, dim=1)
        f = -u * u_x + nu * u_xx
...
if __name__ == "__main__":
    ...
    ax.plot(x_star, U1_pred[:, -1], 'r--', marker='>', linewidth=2, label='Prediction')
    ax.set_xlabel('$x$')
    ax.set_ylabel('$u(t,x)$')
