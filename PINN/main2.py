...
class PhysicsInformedNN():
    def __init__(self, X_u, u, X_f, layers, lb, ub, nu):
        ...
        #self.dnn = Net(input, width, length).to(device)
    ...
    def plot(self):
        np.savetxt('tables/training_error_pytorch.csv', self.loss_table)
        plt.plot(self.loss_table)
        plt.xlabel('iterations')
        plt.ylabel('error')
        plt.title('training error(pytorch)')
        #plt.savefig('figures/training_error_pytorch.pdf')
        plt.show()
...
if __name__ == "__main__":
    ...
    plt.savefig('figures/result1.pdf')
    plt.show()
    ...
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] +
                 ax.get_xticklabels() + ax.get_yticklabels()):
        item.set_fontsize(15)
    plt.savefig('figures/result2.pdf')
    plt.show()
